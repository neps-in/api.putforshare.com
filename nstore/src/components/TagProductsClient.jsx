...
export default function TagProductsClient({ initialSlug, initialPageData = null, initialPage = 1 }) {
  const placeholderImage = "/assets/placeholder-product.svg";
  const router = useRouter();
  const { addToCart } = useCart();
  const tagSlug = initialSlug || "";
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    fetchProductsByTag(tagSlug, { page, pageSize: 24 })
      .then(setPageData)
      .catch(() => setError("Could not load tag products"))
      .finally(() => setLoading(false));
  }, [page, initialPage, initialPageData, tagSlug]);
  ...
  return (
    <main className="w-full px-4 py-8">
      <nav className="flex items-center gap-2 text-sm text-slate-500" aria-label="Breadcrumb">
        ...
        <span>/</span>
        <Link className="font-semibold text-orange-700 hover:text-orange-800" href="/tags">
          Tags
        </Link>
        <span>/</span>
        <span>{tagSlug}</span>
      </nav>
      <h1 className="mt-3 text-2xl font-semibold capitalize text-slate-900">{tagSlug}</h1>
      <p className="mt-1 text-sm text-slate-500">Products tagged with {tagSlug}.</p>
      {error ? <p className="mt-2 text-sm font-semibold text-red-600">{error}</p> : null}
      {loading ? (
        <section className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-2 lg:grid-cols-3" aria-label="loading tag products">
