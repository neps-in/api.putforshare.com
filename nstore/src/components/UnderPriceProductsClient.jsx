...
import { fetchUnderPriceProducts } from "@/lib/api";
import { useCart, useStorefront } from "@/components/ClientShell";
import PaginationControls from "@/components/PaginationControls";

export default function UnderPriceProductsClient({ maxPrice = 10, initialPageData = null, initialPage = 1 }) {
  ...
  useEffect(() => {
    if (page === initialPage && initialPageData && maxPriceValue === maxPrice) {
      return;
    }
    setLoading(true);
    setError("");
    fetchUnderPriceProducts({ page, pageSize: 24, maxPrice: maxPriceValue })
      .then(setPageData)
      .catch(() => setError("Could not load 10 Rs Store products"))
      .finally(() => setLoading(false));
  }, [page, maxPriceValue, initialPage, initialPageData, maxPrice]);
  ...
  return (
    <main className="w-full px-4 py-8">
      <nav className="flex items-center gap-2 text-sm text-slate-500" aria-label="Breadcrumb">
        ...
        <span>/</span>
        <span>10 Rs Store</span>
      </nav>
      <h1 className="mt-3 text-2xl font-semibold text-slate-900">10 Rs Store</h1>
      <p className="mt-1 text-sm text-slate-500">Products priced at Rs {maxPriceValue} or less.</p>

      <section className="mt-4 flex flex-wrap items-center gap-3" aria-label="10 rs store product controls">
        <select
          value={sortBy}
        ...
        <label className="grid gap-1 text-xs font-semibold text-slate-600">
          <span>Max Rs {maxPriceValue}</span>
          <input
            type="range"
            min="1"
            max="500"
      ...
      {loading ? (
        <section className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-2 lg:grid-cols-3" aria-label="loading 10 rs store products">
          {Array.from({ length: 6 }).map((_, index) => (
            <article className="min-h-[170px] animate-pulse rounded-2xl bg-slate-200" key={index} />
          ))}
        </section>
      ) : filteredProducts.length === 0 ? (
        <p className="mt-4 text-sm text-slate-500">No products available in this price range.</p>
      ) : (
        <>
