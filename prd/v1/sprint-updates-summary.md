...
- Non-SELLER users cannot access listing creation
...
- Tags can be added to any listing
...
- `DELETE /api/categories/:id` - Delete category (requires ADMIN role)
  - Checks for active listings in category AND all subcategories
...
- `GET /api/tags/:slug/listings` - Get listings with specific tag
...
- `POST /api/listings` - Now accepts `tags: ["wireless", "bluetooth", "warranty"]`
- `PUT /api/listings/:id` - Can update tags array
...
- Show top 3 tags as pills on listing cards
...
- Category deletion prevention (when subcategories or listings exist)
...
- Related listings algorithm considers shared tags

**Frontend:**
- Popular tags section in filter sidebar
- Tag pills on listing cards (top 3)
...
| Sprint 2 | Listings, Categories & Tags | 52 | +8 |
...
2. Add tags field to Listing model
